...
pub const CONVICTION_LEN: usize = 1 + 32 + 64 + 8 + 8 + 8 + 1;
...
impl ConvictionAccount {
    pub fn write(&self, into: &mut [u8]) -> Result<(), SlashError> {
        ...
        out[97..105].copy_from_slice(&self.slashed.to_le_bytes());
        out[105..113].copy_from_slice(&self.owed_to_victim.to_le_bytes());
        out[113..121].copy_from_slice(&self.slot.to_le_bytes());
        out[121] = self.bump;
        Ok(())
    }
    ...
    pub fn read(from: &[u8]) -> Result<Self, SlashError> {
        ...
        Ok(Self {
            ...
            slashed: u64::from_le_bytes(arr(97, 8).try_into().unwrap()),
            owed_to_victim: u64::from_le_bytes(arr(105, 8).try_into().unwrap()),
            slot: u64::from_le_bytes(arr(113, 8).try_into().unwrap()),
            bump: data[121],
        })
    }
}
